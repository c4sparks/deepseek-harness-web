...
| [02-架构与运行机制](02-架构与运行机制.md) | 设计 | 改结构/连接受 |
| [03-界面交互与用户手册](03-界面交互与用户手册.md) | 交互 | 改 UI |
| [04-dsh协议耦合与升级](04-dsh协议耦合与升级.md) | 协议 | dsh 升级 |
| [05-测试发布与工程](05-测试发布与工程.md) | 发布 | 发版/迭代 |
| [06-对话区设计](06-对话区设计.md) | 设计 | 信息块「在哪/何时/如何显示」唯一基准（改前必读）；行类型/状态体系/工具卡规则 |
| [07-UI规格](07-UI规格.md) | 设计 | 改样式 / 调间距字号圆角 |
...
### 0.1.8（2026-09-11）
- 修改：会话归属硬约束，杜绝「未分组」—— `newSession()` 不再以无参 `createSession({})` 回退；取不到工作区（无 dsh 工作区且无当前文件夹）抛 `DshNoWorkspaceError`，交互入口（＋新会话/发消息/切模型模式权限/斜杠）提示选工作区并弹工作区面板，`listModels()` 不再为列模型强建会话。另删死代码 `stream.ts` 的 `ask()`（裸 `createSession()`）。记录：`01` F4/字段要点、`02` 关键设计点、`03` 用户手册第 14 条。
- 新增：术语表，统一对话区用词并确立标识符命名前缀规则；文档地图补上此前遗漏的组件化边界文档行。
...
- 修改：既有文档用词统一为术语表口径（「过程链」「折叠窗口」等改称「回合过程」「过程折叠」），涉及 `07` 与术语表。
- 修改：`webview/chat/core/store/chat.ts` 拆分（第一阶段）—— 1006 行降为约 600 行的聚合层，摘出 `composer`/`catalogs`/`selectors`/`question`/`status` 五个叶切片与 `types.ts`；`ChatStore` 接口与 `createChatStore` 签名不变，17 个组件消费者零改动。协议类型共享暂缓（与「dsh 协议永不进入组件源码」的边界相悖）。【2026-09-12 修正：原文写「理由记入 `11-术语表` §已知缺口」】
...
- 新增：`02-架构与运行机制` 补「整体架构」小节（整体分层框图 + 一轮对话往返流程框图）与 webview 聊天页模块表。
- 修改：根 `CHANGELOG.md` 补 `0.1.7` 详细条目；`00` §1.3 修正 CHANGELOG 路径（`docs/CHANGELOG.md` 不存在，实为根 `CHANGELOG.md`）。
- 修改：清掉 `design/` 里指向仓库外文档与 `tmp/` 的引用共 14 处 —— 对克隆仓库的人是死链；同轮把 `03` 的 0.1.6 发布记录展开为自足条目（原写「细节见上 0.1.6-rc.2 段」）。规则固化入 `00` §1.7「文档引用方向与自足」（单向引用，反向不可）。
